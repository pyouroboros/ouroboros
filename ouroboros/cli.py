...
def parse(sysargs):
    ...
    parser.add_argument('-m', '--monitor', nargs='+', default=environ.get('MONITOR', []) , dest='monitor',
    ...
    parser.add_argument('-r', '--runonce', default=environ.get('RUNONCE', False), dest='run_once',
                        help='Only run ouroboros once then exit', action='store_true')

    parser.add_argument('-c', '--cleanup', default=environ.get('CLEANUP', False), dest='cleanup',
                        help='Remove old images after updating', action='store_true')

    parser.add_argument('-k', '--keep-tag', default=environ.get('KEEPTAG', False), dest='keep_tag',
